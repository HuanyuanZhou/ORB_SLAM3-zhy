...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    //mpTracker中跟踪进行的主要函数，在此前imu数据已被存至mpTracker->mlQueueImuData，数据结构为list
...
{
    return mpTracker->mbNeedKF;
}

bool System::GetIfReset()
{
    return mbReset;
}

bool System::GetIfResetActiveMap()
{
...
void System::SaveTrajectoryEuRoC(const string &filename)
{
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO)
        {
            ...
            cv::Mat Rwc = Tcw.rowRange(0,3).colRange(0,3).t();
            cv::Mat twc = -Rwc*Tcw.rowRange(0,3).col(3);
            vector<float> qc = Converter::toQuaternion(Rwc);
            fc << setprecision(6) << 1e9*(*lT) << " " << setprecision(9) << twc.at<float>(0) << " " << twc.at<float>(1) << " " << twc.at<float>(2) << " " <<
                    qc[0] << " " << qc[1] << " " << qc[2] << " " << qc[3] << endl;
        }
        else
        {
            cv::Mat Tcw = (*lit)*Trw;
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
       ...
       // pKF->SetPose(pKF->GetPose()*Two);
        ...
        if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO)
        {
            ...
            cv::Mat twc = -Rwc*Tcw.rowRange(0,3).col(3);
            vector<float> qc = Converter::toQuaternion(Rwc);
            kfc << setprecision(6) << 1e9*pKF->mTimeStamp << " " << setprecision(9) << twc.at<float>(0) << " " << twc.at<float>(1) << " " << twc.at<float>(2) << " " <<
                    qc[0] << " " << qc[1] << " " << qc[2] << " " << qc[3] << endl;
        }
        else
        {
